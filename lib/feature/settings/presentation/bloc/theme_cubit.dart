...
class ThemeCubit extends Cubit<ThemeState> {
  ...
  ThemeCubit(this.themeRepository, this.toggleTheme) : super(ThemeState(AppTheme.light, AppThemes.lightTheme)) {
  ...
  Future<void> _loadSavedTheme() async {
    final savedTheme = await themeRepository.loadTheme(); // ✅ Doğrudan çağırıyoruz
    emit(ThemeState(savedTheme, savedTheme == AppTheme.dark ? AppThemes.darkTheme : AppThemes.lightTheme));
  }

  Future<void> toggle() async {
    final newTheme = await toggleTheme(state.appTheme);
    emit(ThemeState(newTheme, newTheme == AppTheme.dark ? AppThemes.darkTheme : AppThemes.lightTheme));
  }
}
